...
if __name__ == '__main__':
    ...
    # Parser #
    ##########
    input_dir = './data/dnsmasq/'
    output_dir = './dnsmasq_result/'
    # log_format = '<Logrecord> <Date> <Time> <Pid> <Level> <Component> \[<ADDR>\] <Content>'
    log_format= "<Date> <Time> <Ident> <Pid> <Content>"
    log_main = 'dnsmasq'
    tau = 0.5
    ...
    for log_name in ['dns_anomaly1.log', 'dns_normal2.log', 'dns_normal1.log']:
        parser.parse(log_name)

    ##################
    # Transformation #
    ##################
    df = pd.read_csv(f'{output_dir}dns_anomaly1.log_structured.csv')
    df_normal = pd.read_csv(f'{output_dir}dns_normal2.log_structured.csv')
    df_abnormal = pd.read_csv(f'{output_dir}dns_anomaly1.log_structured.csv')
